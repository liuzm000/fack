...
<script type="text/javascript">
	function search(){
		document.forms["searchForm"].action = "getInterfaceQuery.action";
		document.forms["searchForm"].submit();
	}

	function add(){
		document.forms["searchForm"].action = "doInterfaceAdd.action";
		document.forms["searchForm"].submit();
		
	}

	function edit(obj){
		document.forms["searchForm"].action = "doInterfaceEdit.action?id=" + obj;
		document.forms["searchForm"].submit();
	}
	
	function del(obj,obj1){
		if(confirm("你确定要删除这条记录吗？")){
			document.forms["searchForm"].action = "doInterfaceDelete.action?id=" + obj+"&url="+obj1;
			document.forms["searchForm"].submit();
		}
	}
</script>
...
<div class="top">
	...
	<img src="../images/icon/shuju.gif" /><span class="title">接口管理 - 列表</span>
</div>
<!-- TOP END -->
	...
	<div class="tablebox">
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listTab">
			<tr>
				<th width="10%">接口id</th>
				<th width="10%">接口编码</th>
				<th width="10%">父类接口编码</th>
				<th width="15%">对应接口类型</th>
				<th width="15%">接口名称</th>
				<th width="15%">操作</th>
			</tr>
			<#if pageList?exists>
			...
			<tr onMouseOver="this.className='row_over'" onMouseOut="this.className='row_out'">
				...
				<td align="center">${page.interNum?if_exists}</td>
				<#if page.parentInterNum?exists>
					<td align="center">${page.parentInterNum?if_exists}</td>
				<#else>
					<td align="center">无</td>
				</#if>
				<td align="center">${page.interName?if_exists}</td>
			...
			<tr>
				...
				<#if pageList.recordCount?exists>
					...
					<@pg.pager isOffset=true encoder="utf-8" maxPageItems=pageSize maxIndexPages=maxindexpages items=recordCount url="${actionName?if_exists}" export="offset,currentPageNumber=pageNumber" scope = "request">
						<@pg.index>
							<@pg.param name="msgModel.title"  value="${model?if_exists.descript?if_exists}"/>
