...
<table width="100%" height="99%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td width="94%" height="36" background="../images/main/all_t_bg.gif"><img src="../images/icon/baobiao.gif"><span class="title"><strong>短信管理</strong> -短信发送记录查询</span></td>
  ...
  <tr>
    ...
    <form id="searchForm" name="searchForm" method="post" action="${request.contextPath}/sms/smsHisSearch.action">
    <table width="98%" border="0" cellspacing="0" cellpadding="0" class="boxsearch">
      ...
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listTab" style="WORD-BREAK:break-all">
        ...
        <tr onMouseOver="this.className='row_over'" onMouseOut="this.className='row_out'">
          ...
          <td width="35%"><div align="center"><strong>短信内容</strong></div></td>
          <td width="20%"><div align="center"><strong>发送时间</strong></div></td>
		  <td width="10%" ><div align="center"><strong>状态</strong></div></td>
		  <td width="15%"><div align="center"><strong>用户账号</strong></div></td>
        </tr>
        ...
        <tr onMouseOver="this.className='row_over'" onMouseOut="this.className='row_out'">
          ...
          <td><div align="center">${vo.content?if_exists}</div></td>
		  <td><div align="center">${vo.sendTime?if_exists}</div></td>
		  <td><div align="center">
		  <#if vo.receiptStatus?exists && vo.receiptStatus != "">
		  	<#if vo.receiptStatus == '000'>
		  	发送成功
		  	<#else>
		  	发送失败
		    </#if>
		  <#elseif vo.respStatus?exists && vo.respStatus != "">
		  	<#if vo.respStatus == '0'>
		  	已发送
		  	<#else>
		  	系统忙
		  ...
		  <td><div align="center">${vo.userAccount?if_exists}</div>
          </td>
        </tr>
        </#list>
        </#if>
          ...
          <td bgcolor="#FFFFFF" style="width:100%" colspan="6" align="center">  
				...
				<#if dataList?exists>
					<#if dataList.recordCount?exists>
						...
						<@pg.pager isOffset=true encoder="UTF-8" maxPageItems=pageSize maxIndexPages=maxindexpages items=recordCount url="${request.contextPath}/sms/${actionName?if_exists}" export="offset,currentPageNumber=pageNumber" scope = "request">
						<@pg.index>		
