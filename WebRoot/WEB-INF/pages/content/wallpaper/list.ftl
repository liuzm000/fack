...
function addNew(){
	window.location.href = "${rootPath}/content/wallpaper/preadd.action";
}
...
<table width="100%" height="99%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td width="94%" height="36" background="${rootPath}/images/main/all_t_bg.gif"><img src="${rootPath}/images/icon/baobiao.gif"><span class="title"><strong>桌面背景管理</strong> - 查看列表</span></td>
  ...
  <tr>
    <td colspan="3" align="center" valign="top" bgcolor="#FFFFFF" class="allbox">
      ...
      <tr>
          ...
          <td width="30%" align="center"><b>文件路径</td>
        ...
        <tr onMouseOver="this.className='row_over'" onMouseOut="this.className='row_out'" align="center">
          <td id="pluginId" align="center">${vo.id?if_exists}</td>
          ...
          <td align="center">
          <a href="${rootPath}/content/wallpaper/delete.action?id=${vo.id?if_exists}" class="redfontvha">删除</a>
          </td>
        </tr>
        </#list>
        </#if>
          ...
          <td bgcolor="#FFFFFF" style="width:100%" colspan="6" align="center">  
				...
				<#if dataList?exists>
					<#if dataList.recordCount?exists>
						...
						<@pg.pager isOffset=true encoder="UTF-8" maxPageItems=pageSize maxIndexPages=maxindexpages items=recordCount url="${request.contextPath}/content/wallpaper/list.action" export="offset,currentPageNumber=pageNumber" scope = "request">
						<@pg.index>			
							<@pg.param name="wallpaper.price"  value="${wallpaper?if_exists.price?if_exists}"/>  		 
