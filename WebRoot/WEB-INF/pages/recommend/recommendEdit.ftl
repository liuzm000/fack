...
<script>
	function checkUpdateForm(){
		...
		document.forms["editForm"].action = "updateHomeRecommend.action";
...
<body>
  ...
  <div class="topleft"></div>
    <img src="../images/icon/baobiao.gif"><span class="title"><strong>首页推荐位(3.3版)信息 - 更新</strong></span>
  </div>
  <!-- 主体部分 -->
  <div class="tmiddle">&nbsp;
  	<div class="boxsearch">
  	...
  	<div class="boxsearch">
  		...
  		<table width="80%" border="0" align="center" cellpadding="0" cellspacing="0" class="tab">
  				<tr>
					<td width="20%" align="right" style="padding-right:10px"><b>ID：</b></td>
					<td width="30%" align="left" style="padding-left:10px">${vo.hrId?if_exists}
					<input id="hrModel.hrId" name="hrModel.hrId"  maxlength="50" size="60" type="hidden" value="${vo.hrId?if_exists}"/>
					</td>
					<td width="40%" align="left"><font color="green"></font></td>
				</tr>
				<tr>
					...
					<input id="hrModel.subjectName" name="hrModel.subjectName"  maxlength="50" size="40" type="text" value="${vo.subjectName?if_exists}"/>
					</td>
				...
				<tr>
					...
					<select name="hrModel.subjectType" id="hrModel.subjectType" style="width:150px">
						<option value="0" <#if 0 == vo.subjectType>selected</#if>>无点击动作</option>
						<option value="1" <#if 1 == vo.subjectType>selected</#if>>客户端内嵌页</option>
						<option value="2" <#if 2 == vo.subjectType>selected</#if>>WAP页面</option>
						<option value="3" <#if 3 == vo.subjectType>selected</#if>>应用详情页</option>
						<option value="4" <#if 4 == vo.subjectType>selected</#if>>专题应用列表页</option>
					</select>
					</td>
				...
				<tr>
					...
					<td align="left" style="padding-left:10px">
					<input id="hrModel.subjectId" name="hrModel.subjectId"  maxlength="200" size="40" type="text" value="${vo.subjectId?if_exists}"/>
					</td>
				...
				<tr>
					...
					<td align="left" style="padding-left:10px">
					<input id="hrModel.subjectWapUrl" name="hrModel.subjectWapUrl"  maxlength="200" size="60" type="text" value="${vo.subjectWapUrl?if_exists}"/>
					</td>
				...
				<tr>
					...
					<option value="1" <#if 1 == vo.appTypeId>selected</#if>>最新</option>
					<option value="2" <#if 2 == vo.appTypeId>selected</#if>>最热</option>
					<option value="3" <#if 3 == vo.appTypeId>selected</#if>>金融导航</option>
					<!--<#if appTypeList?exists>
					<#list appTypeList as cl>
						<option value="${cl.appTypeId?if_exists}" <#if cl.appTypeId == vo.appTypeId>selected</#if> >${cl.appTypeName?if_exists}</option>
					</#list>
					</#if>-->
					</select>
					</td>
				...
				<tr id="iconImgTr" style="">
					<td align="right" style="padding-right:10px"><b>图片文件预览</b></td>
					<td align="left" style="padding-left:10px" colspan=2>
					<#if vo.appTypeId != -1>
					<img id="iconImg" src="${rootPath}${vo.subjectIconUrl?if_exists}" width="224" height="79" style=""/>
					<#else>
					<img id="iconImg" src="${rootPath}${vo.subjectIconUrl?if_exists}" width="80" height="80" style=""/>
					</#if>
					</td>
				</tr>
				<tr>
					...
					<select name="hrModel.subjectWapUrl" id="hrModel.subjectWapUrl" style="width:150px">
						<option value="0" <#if vo.subjectWapUrl == 0>selected</#if>>无状态</option>
						<option value="3" <#if vo.subjectWapUrl == 3>selected</#if>>最新</option>
					    <option value="2" <#if vo.subjectWapUrl == 2>selected</#if>>加精</option>
					    <option value="1" <#if vo.subjectWapUrl == 2>selected</#if>>热门</option>
					</select>
					</td>
		...
		<div class="boxsearch2">
			<a href="javascript:checkUpdateForm()"><img src="../images/buttom/buttom_baocun.gif" alt="保存" /></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:window.history.back()'><img src="../images/buttom/buttom_fanhui.gif" alt="返回"/></a>
		</div>
	  	</form>
  	</div>
  </div>
