...
<script type="text/javascript">
	...
	function search(){
		document.forms["searchForm"].action = "getEcotourismTypeList.action";
		document.forms["searchForm"].submit();
		
	}
	function add(){
		document.forms["searchForm"].action = "toAddEcotourismType.action";
		document.forms["searchForm"].submit();
		
	}
	function toEcotourismTypeEdit(obj){
		document.forms["searchForm"].action = "toEcotourismTypeEdit.action?typeId="+obj;
		document.forms["searchForm"].submit();
		
	}
	
	function viewEcotourismList(obj){
		document.forms["searchForm"].action = "viewEcotourismList.action?typeId="+obj;
		document.forms["searchForm"].submit();
	...
	function del(obj){
		if(confirm("你确定要删除这条记录吗？")){
			document.forms["searchForm"].action = "doEcotourismTypeDelete.action?typeId=" + obj;
			document.forms["searchForm"].submit();
		}
	}
</script>
...
<div class="tmiddle">&nbsp;
	<div class="boxsearch">
		<div class="boxsearch2">
			<form id="searchForm" name="searchForm" method="post" action="getTownPictureList.action">
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listTab">
			...
			<#if pageList?exists>
				...
				<tr onMouseOver="this.className='row_over'" onMouseOut="this.className='row_out'">
					...
					<td align="center">
						<img src="${rootPath}/download${page.iconPath?if_exists}" width="48" height="48"/>
					</td>
					<td align="center">${page.typeName?if_exists}</td>
					<td align="center">
						<#if page.infoHtml?exists>
							<a style="cursor: pointer;" href="${rootPath}/download${page.infoHtml?if_exists}"><font color="blue"><u>查看</u></font></a>
						</#if>
					</td>
					<td align="center">
						<#if page.filePaths?exists>
							<#list page.filePaths?split(",") as s>
					...
					<td align="center">
					<a href="javascript:viewEcotourismList('${page.id?if_exists}')"><font color="blue"><u>查看图集</u></font></a>
					<a href="javascript:toEcotourismTypeEdit('${page.id?if_exists}')"><font color="blue"><u>修改</u></font></a>
