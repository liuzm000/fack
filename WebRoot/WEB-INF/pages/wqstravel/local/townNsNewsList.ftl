...
<script type="text/javascript">
	function search(){
		document.forms["searchForm"].action = "getTownNsNewsList.action?typeId=2";
		document.forms["searchForm"].submit();
	}

	function add(){
		document.forms["searchForm"].action = "doTownNsNewsAdd.action";
		document.forms["searchForm"].submit();
		
	}

	function edit(obj){
		document.forms["searchForm"].action = "doTownNsNewsEdit.action?id=" + obj;
		document.forms["searchForm"].submit();
	}
	
	function del(obj,obj1){
		if(confirm("你确定要删除这条记录吗？")){
			document.forms["searchForm"].action = "doTownNsNewsDelete.action?id=" + obj+"&url="+obj1+"&typeId=2";
			document.forms["searchForm"].submit();
		}
	}
</script>
...
<div class="top">
	...
	<img src="../images/icon/shuju.gif" /><span class="title">南沙新闻 - 列表</span>
</div>
<!-- TOP END -->
