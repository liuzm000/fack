...
<script type="text/javascript">
	function search(){
		document.forms["searchForm"].action = "getHospitalList.action";
		document.forms["searchForm"].submit();
	}

	function add(){
		document.forms["searchForm"].action = "doHospitalAdd.action";
		document.forms["searchForm"].submit();
		
	}

	function edit(obj){
		document.forms["searchForm"].action = "toHospitalEdit.action?id=" + obj;
		document.forms["searchForm"].submit();
	}
	
	function del(obj,obj1){
		if(confirm("你确定要删除这条记录吗？")){
			document.forms["searchForm"].action = "doHospitalDelete.action?id=" + obj+"&url="+obj1;
			document.forms["searchForm"].submit();
		}
	}
</script>
...
<div class="top">
	...
	<img src="../images/icon/shuju.gif" /><span class="title">医疗服务 - 列表</span>
</div>
<!-- TOP END -->
		...
		<div class="boxsearch2">
			...
			<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="">
				<tr>
					...
					<td width="10%" align="left">
						<input id="searchUnit" name="searchAdd" value="${searchAdd?if_exists}" maxlength="50" size="15" type="text" />
					</td>
	...
	<div class="tablebox">
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listTab">
			...
			<tr onMouseOver="this.className='row_over'" onMouseOut="this.className='row_out'">
				...
				<td align="center">${page.address?if_exists}</td>
				<td align="center">${page.tele1?if_exists}</td>
				<td align="center">${page.tele2?if_exists}</td>
				<td align="center">${page.tele3?if_exists}</td>
				<td align="center"><a style="cursor: pointer;" href="${rootPath}/download${page.htmlFile?if_exists}"><font color="blue">查看</font></a></td>
				<td align="center">${page.htmlFile?if_exists}</td>
				<td align="center">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="box_fanyeSS
			<tr>
				<td>
				<!--分页-->
				<#if pageList?exists>
				<#if pageList.recordCount?exists>
					...
					<@pg.pager isOffset=true encoder="utf-8" maxPageItems=pageSize maxIndexPages=maxindexpages items=recordCount url="${actionName?if_exists}" export="offset,currentPageNumber=pageNumber" scope = "request">
						<@pg.index>
							...
							<@pg.param name="searchAdd"  value="${searchAdd?if_exists}"/>
