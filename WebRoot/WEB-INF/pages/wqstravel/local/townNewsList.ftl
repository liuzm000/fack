...
<script type="text/javascript">
	function search(){
		document.forms["searchForm"].action = "getTownNewsList.action?typeId=1";
		document.forms["searchForm"].submit();
	}

	function add(){
		document.forms["searchForm"].action = "doTownNewsAdd.action";
		document.forms["searchForm"].submit();
		
	}

	function edit(obj){
		document.forms["searchForm"].action = "doTownNewsEdit.action?id=" + obj;
		document.forms["searchForm"].submit();
	}
	
	function del(obj,obj1){
		if(confirm("你确定要删除这条记录吗？")){
			document.forms["searchForm"].action = "doTownNewsDelete.action?id=" + obj+"&url="+obj1+"&typeId=1";
			document.forms["searchForm"].submit();
		}
	}
</script>
...
<div class="top">
	...
	<img src="../images/icon/shuju.gif" /><span class="title">万顷沙新闻 - 列表</span>
</div>
<!-- TOP END -->
