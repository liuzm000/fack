...
<script type="text/javascript">
	function search(){
		document.forms["searchForm"].action = "getTownJobsList.action";
		document.forms["searchForm"].submit();
	}

	function add(){
		document.forms["searchForm"].action = "doTownJobsAdd.action";
		document.forms["searchForm"].submit();
		
	}

	function edit(obj){
		document.forms["searchForm"].action = "doTownJobsEdit.action?id=" + obj;
		document.forms["searchForm"].submit();
	}
	
	function del(obj,obj1){
		if(confirm("你确定要删除这条记录吗？")){
			document.forms["searchForm"].action = "doTownJobsDelete.action?id=" + obj+"&url="+obj1;
			document.forms["searchForm"].submit();
		}
	}
</script>
...
<div class="top">
	...
	<img src="../images/icon/shuju.gif" /><span class="title">招聘信息 - 列表</span>
</div>
<!-- TOP END -->
	...
	<div class="boxsearch">
		<div class="boxsearch2">
			...
			<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="">
				<tr>
					<td width="10%" align="right">主题名：</td>
					...
					<td width="10%" align="left">
						<input id="searchUnit" name="searchUnit" value="${searchUnit?if_exists}" maxlength="50" size="15" type="text" />
					</td>
	...
	<div class="tablebox">
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listTab">
			...
			<tr onMouseOver="this.className='row_over'" onMouseOut="this.className='row_out'">
				...
				<td align="center">${page.contactUser?if_exists}</td>
				<td align="center">${page.contactPhone?if_exists}</td>
				<td align="center"><a style="cursor: pointer;" href="${rootPath}/download${page.jobUrl?if_exists}"><font color="blue">查看</font></a></td>
				<td align="center"><a style="cursor: pointer;" href="${rootPath}/download${page.infoUrl?if_exists}"><font color="blue">查看</font></a></td>
				...
				<#if pageList.recordCount?exists>
					...
					<@pg.pager isOffset=true encoder="utf-8" maxPageItems=pageSize maxIndexPages=maxindexpages items=recordCount url="${actionName?if_exists}" export="offset,currentPageNumber=pageNumber" scope = "request">
						<@pg.index>
							...
							<@pg.param name="searchUnit"  value="${searchUnit?if_exists}"/>
