...
<script type="text/javascript">
	function search(){
		document.forms["searchForm"].action = "getTownGnNewsList.action?typeId=3";
		document.forms["searchForm"].submit();
	}

	function add(){
		document.forms["searchForm"].action = "doTownGnNewsAdd.action";
		document.forms["searchForm"].submit();
		
	}

	function edit(obj){
		document.forms["searchForm"].action = "doTownGnNewsEdit.action?id=" + obj;
		document.forms["searchForm"].submit();
	}
	
	function del(obj,obj1){
		if(confirm("你确定要删除这条记录吗？")){
			document.forms["searchForm"].action = "doTownGnNewsDelete.action?id=" + obj+"&url="+obj1+"&typeId=3";
			document.forms["searchForm"].submit();
		}
	}
</script>
...
<div class="top">
	...
	<img src="../images/icon/shuju.gif" /><span class="title">国内新闻 - 列表</span>
</div>
<!-- TOP END -->
