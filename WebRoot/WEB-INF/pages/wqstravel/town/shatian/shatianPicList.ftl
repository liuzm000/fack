...
<script type="text/javascript">
	...
	function del(obj,typeId){
		if(confirm("你确定要删除这条记录吗？")){
			document.forms["searchForm"].action = "doTownOnePictureDelete.action?id=" + obj+"&typeId="+typeId;
			document.forms["searchForm"].submit();
		}
	}
	
	function edit(obj,typeId){
		document.forms["searchForm"].action = "toPicEdit.action?id=" + obj+"&typeId="+typeId;
		document.forms["searchForm"].submit();
	}
</script>
	...
	<div class="boxsearch">
		<div class="boxsearch2">
			...
			<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="">
				<tr>
					<td colspan="8">
					<#--<a href="javascript:toAddPicList('${id?if_exists}')"><img src="../images/buttom/buttom_xinzen.gif" /></a>-->
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listTab">
			...
			<#if pageList?exists>
				...
				<tr onMouseOver="this.className='row_over'" onMouseOut="this.className='row_out'">
					...
					<td align="center">
					<a href="javascript:edit('${page.id?if_exists}','${id?if_exists}')"><font color="blue"><u>修改</u></font></a>
					<a href="javascript:del('${page.id?if_exists}','${id?if_exists}')"><font color="red"><u>删除</u></font></a>
					</td>
				</tr>
				</#list>
			</#if>
			...
			<tr>
				...
				<#if pageList.recordCount?exists>
					...
					<@pg.pager isOffset=true encoder="utf-8" maxPageItems=pageSize maxIndexPages=maxindexpages items=recordCount url="${actionName?if_exists}" export="offset,currentPageNumber=pageNumber" scope = "request">
						<@pg.index>
							<@pg.param name="id"  value="${id?if_exists}"/>
