...
<script type="text/javascript">
	function add(){
		document.forms["searchForm"].action = "doTownEntAdd.action";
		document.forms["searchForm"].submit();
		
	}
	
	function upload(obj,obj1){
		document.forms["searchForm"].action = "doTownEntUpload.action?id=" + obj + "&name=" + obj1;
		document.forms["searchForm"].submit();
	}

	function edit(obj){
		document.forms["searchForm"].action = "doTownEntEdit.action?id=" + obj;
		document.forms["searchForm"].submit();
	}
	...
	function search(){
		document.forms["searchForm"].action = "getTownEntList.action";
		document.forms["searchForm"].submit();
...
<div class="top">
	...
	<img src="../images/icon/shuju.gif" /><span class="title">企业名录 - 列表</span>
</div>
<!-- TOP END -->
	...
	<div class="tablebox">
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listTab">
			...
			<tr onMouseOver="this.className='row_over'" onMouseOut="this.className='row_out'">
				...
				<td align="center"><a style="cursor: pointer;" href="${rootPath}/download${page.htmlPath?if_exists}"><font color="blue">查看</font></a></td>
				<td align="left" class="wrap">
					<#list page.icon?if_exists?split(",") as x>
					<#if x_index == 3>
						……
						<#break>
					</#if>
					<#if x != "">
					 <img width="100px" height="100px" style="cursor: pointer;" src="${rootPath}/download${x}"/>
					 </#if>
					</#list>
				</td>
				<td align="center">
				<a href="javascript:upload('${page.id?if_exists}','${page.name?if_exists}')"><font color="blue"><u>插图上传</u></font></a>
