...
<script type="text/javascript">
	function search(){
		document.forms["searchForm"].action = "getTownVideoQuery.action";
		document.forms["searchForm"].submit();
	}

	function add(){
		document.forms["searchForm"].action = "doTownVideoAdd.action";
		document.forms["searchForm"].submit();
		
	}

	function edit(obj){
		document.forms["searchForm"].action = "doTownVideoEdit.action?id=" + obj;
		document.forms["searchForm"].submit();
	}
	
	function del(obj,obj1){
		if(confirm("你确定要删除这条记录吗？")){
			document.forms["searchForm"].action = "doTownVideoDelete.action?id=" + obj+"&url=" + obj1;
			document.forms["searchForm"].submit();
		}
	}
...
<div class="top">
	...
	<img src="../images/icon/shuju.gif" /><span class="title">视频管理 - 列表</span>
</div>
<!-- TOP END -->
		...
		<div class="boxsearch2">
			...
			<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="">
				...
				<tr>
					...
					<input id="videoModel.title" name="videoModel.title" value="${model?if_exists.title?if_exists}"  maxlength="50" size="15" type="text" value=""/>
					</td>
	...
	<div class="tablebox">
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listTab">
			...
			<tr onMouseOver="this.className='row_over'" onMouseOut="this.className='row_out'">
				...
				<td align="center"><font color="blue">${page.title?if_exists}</td>
				<td align="center">${page.filePath?if_exists}&nbsp;&nbsp;
				<a href="javascript:download('${page.id?if_exists}','${page.filePath?if_exists}')"><font color="red"><u>下载</u></font></a>
				</td>
				<td align="center">${page.author?if_exists}</td>
				<td align="center">${page.createTime?if_exists}</td>
				<td align="center">
				<a href="javascript:edit('${page.id?if_exists}')"><font color="blue"><u>修改</u></font></a>
				<a href="javascript:del('${page.id?if_exists}','${page.filePath?if_exists}')"><font color="red"><u>删除</u></font></a>
				</td>
			</tr>
			</#list>
			</#if>
			...
			<tr>
				...
				<#if pageList.recordCount?exists>
					...
					<@pg.pager isOffset=true encoder="utf-8" maxPageItems=pageSize maxIndexPages=maxindexpages items=recordCount url="${actionName?if_exists}" export="offset,currentPageNumber=pageNumber" scope = "request">
						<@pg.index>
							<@pg.param name="videoModel.title"  value="${model?if_exists.title?if_exists}"/>
