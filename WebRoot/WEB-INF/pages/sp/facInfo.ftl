...
<div class="boxsearch">
  ...
  <form id="query" name="query" method="post" action="facInfo.action">
  厂家名称：<input type="text" name="name" id="name" value="${name?if_exists}" />
  &nbsp;&nbsp;&nbsp;
   描述：
      <input type="text" name="description" id="description" value="${description?if_exists}" />
        <div class="grayline"></div>
       <a href="javascript:$('query').submit();" ><img alt="查询" src="../images/buttom/buttom_search.gif"/></a>&nbsp;&nbsp;<a href="facAdd.action"><img alt="新增" src="../images/buttom/buttom_xinzen.gif"/></a>
  </form>
</div>
</div>
<div class="tablebox"><#if reslist?exists>
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listTab">
        ...
        <tr onMouseOver="this.className='row_over'" onMouseOut="this.className='row_out'">
          ...
          <td align="center">${rl.fac_id?if_exists}</td>
          <td align="center">${rl.fac_desc?if_exists}</td>
          <td align="center">
          <a href="facModify.action?id=${rl.fac_id?if_exists}" class="redfontvha">修改</a>  | <a href="javascript:dodel(${rl.fac_id?if_exists})" class="redfontvha">删除</a></td>
     </tr>
        </#list>
     ...
     <table width="100%" border="0" cellpadding="0" cellspacing="0" class="box_fanye">
        <tr><td>
			...
			<#if reslist.recordCount?exists>
				...
				<@pg.pager isOffset=true encoder="utf-8" maxPageItems=pageSize maxIndexPages=maxindexpages items=recordCount url="facInfo.action" export="offset,currentPageNumber=pageNumber" scope = "request">
				<@pg.index>
